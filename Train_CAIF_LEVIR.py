...
from CAIFNet import CAIFNet



def resize_for_a2net(outputs):
    outputs[0] = F.interpolate(outputs[0], scale_factor=(32, 32), mode='bilinear')
    outputs[0] = torch.sigmoid(outputs[0])

    outputs[1] = F.interpolate(outputs[1], scale_factor=(16, 16), mode='bilinear')
    outputs[1] = torch.sigmoid(outputs[1])

    outputs[2] = F.interpolate(outputs[2], scale_factor=(8, 8), mode='bilinear')
    outputs[2] = torch.sigmoid(outputs[2])

    outputs[3] = F.interpolate(outputs[3], scale_factor=(4, 4), mode='bilinear')
...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        e_target_var = torch.autograd.Variable(e_target).float()
...
def trainValidateSegmentation(args):
    ...
    if args.file_root == 'LEVIR':
        args.file_root = '/data/lzh/dataset/levir-256_by_lyk'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_steps', type=int, default=, help='Max. number of iterations')
