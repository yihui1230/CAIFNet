...
class Scale(object):
    ...
    def __call__(self, img, label,e_label):
        '''
        :param img: RGB image
        ...
        e_label = cv2.resize(e_label, (self.w, self.h), interpolation=cv2.INTER_NEAREST)
        return [img, label,e_label]
...
class Resize(object):
    ...
    def __call__(self, image, label,e_label):
        ...
        e_label = cv2.resize(e_label, size, interpolation=cv2.INTER_NEAREST)
        return (image, label,e_label)
...
class RandomCropResize(object):
    ...
    def __call__(self, img, label,e_label):
        if random.random() < 0.5:
            ...
            e_label_crop = e_label[y1:h - y1, x1:w - x1]


            img_crop = cv2.resize(img_crop, (w, h))
            label_crop = cv2.resize(label_crop, (w, h), interpolation=cv2.INTER_NEAREST)
            e_label_crop = cv2.resize(e_label_crop, (w, h), interpolation=cv2.INTER_NEAREST)

            return img_crop, label_crop,e_label_crop
        else:
            return [img, label,e_label]
...
class RandomFlip(object):
    ...
    def __call__(self, image, label,e_label):
        if random.random() < 0.5:
                ...
                e_label = cv2.flip(e_label, 0)  # horizontal flip
        if random.random() < 0.5:
                image = cv2.flip(image, 1)  # veritcal flip
                label = cv2.flip(label, 1)  # veritcal flip
                e_label = cv2.flip(e_label, 1)  # veritcal flip
        return [image, label,e_label]
...
class ToTensor(object):
    ...
    def __call__(self, image, label,e_label):
        if self.scale != 1:
            ...
            e_label = cv2.resize(e_label, (int(w / self.scale), int(h / self.scale)), \
                               interpolation=cv2.INTER_NEAREST)
        ...
        e_label_tensor = torch.LongTensor(np.array(e_label, dtype=np.int64)).unsqueeze(dim=0)

        return [image_tensor, label_tensor,e_label_tensor]
